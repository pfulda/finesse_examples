#!/usr/bin/env python
"""-----------------------------------------------------------------
  Python file for plotting Finesse ouput Tcav_RFjitter.out
  created automatically Fri Oct 30 14:08:08 2015

  Run from command line as: python Tcav_RFjitter.py
  Load from python script as: import Tcav_RFjitter
  And then use:
  Tcav_RFjitter.run() for plotting only
  x,y=Tcav_RFjitter.run() for plotting and loading the data
  x,y=Tcav_RFjitter.run(1) for only loading the data
-----------------------------------------------------------------"""
...
def run(noplot=None):
	data = np.loadtxt('Tcav_RFjitter.out',comments='%')
	rows,cols=data.shape
	x=data[:,0]
	y=data[:,1:cols]
	mytitle='Tcav_RFjitter                Fri Oct 30 14:08:08 2015'
	if (noplot==None):
		...
		ax1.set_xlim(1,1e+07)
		ax1.set_xscale('log',nonposz='clip')
		ax1.set_xlabel('f [Hz] (sweptEOAM)')
		ax1.set_ylabel('Abs ')
		ax1.yaxis.set_major_formatter(formatter)
		ax2 = ax1.twinx()
		trace1=ax1.plot(x, y[:,0], '-', linewidth=pp.LINEWIDTH, color = next(colorcycler), label = 'am_trans nMCTRANS : Abs ')
		trace2=ax2.plot(x, y[:,1], '-', dashes=[8, 4, 2, 4, 2, 4], linewidth=pp.LINEWIDTH, color = next(colorcycler), label = 'am_trans nMCTRANS : Phase [Deg] ')
		trace3=ax1.plot(x, y[:,2], '-', linewidth=pp.LINEWIDTH, color = next(colorcycler), label = 'am_in nMC1in* : Abs ')
		trace4=ax2.plot(x, y[:,3], '-', dashes=[8, 4, 2, 4, 2, 4], linewidth=pp.LINEWIDTH, color = next(colorcycler), label = 'am_in nMC1in* : Phase [Deg] ')
		ax2.set_ylabel('Phase [Deg] ')
		ax2.yaxis.set_major_formatter(formatter)
		traces = trace1 + trace2 + trace3 + trace4
		traces_a = trace1 + trace3
		traces_p = trace2 + trace4
