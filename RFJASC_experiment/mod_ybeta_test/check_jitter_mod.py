...
def run(noplot=None):
	...
	x=data[:,0]
	y=data[:,1:cols]
	mytitle='check_jitter_mod                Wed Dec 30 15:10:50 2015'
	if (noplot==None):
		...
		trace13=ax1.plot(x, y[:,12], '-', linewidth=pp.LINEWIDTH, color = next(colorcycler), label = 'pdRFAMcheck ndets : Abs ')
		trace14=ax2.plot(x, y[:,13], '-', dashes=[8, 4, 2, 4, 2, 4], linewidth=pp.LINEWIDTH, color = next(colorcycler), label = 'pdRFAMcheck ndets : Phase [Deg] ')
		ax2.set_ylabel('Phase [Deg] ')
		ax2.yaxis.set_major_formatter(formatter)
		traces = trace1 + trace2 + trace3 + trace4 + trace5 + trace6 + trace7 + trace8 + trace9 + trace10 + trace11 + trace12 + trace13 + trace14
		traces_a = trace1 + trace3 + trace5 + trace7 + trace9 + trace11 + trace13
		traces_p = trace2 + trace4 + trace6 + trace8 + trace10 + trace12 + trace14
